<template>
  <v-card flat width="1500">
    <v-table density="compact">
      ...
      <tbody>
        <tr v-for="(item, ix) in list" :key="ix">
          ...
          <td>
            <v-badge
              v-if="checkValue(item.tempMo, 'tempMo')"
              dot
              inline
              color="error"
            />{{ item.tempMo }}
          ...
          <td>
            <v-badge
              v-if="checkValue(item.tempEv, 'tempEv')"
              dot
              inline
              color="error"
            />{{ item.tempEv }}
          ...
          <td>
            <v-badge
              v-if="checkValue(item.pulseRateMo, 'pulseRateMo')"
              dot
              inline
              color="error"
            />{{ item.pulseRateMo }}
          ...
          <td>
            <v-badge
              v-if="checkValue(item.pulseRateEv, 'pulseRateEv')"
              dot
              inline
              color="error"
            />{{ item.pulseRateEv }}
          ...
          <td>
            <v-badge
              v-if="checkValue(item.oxygenMo, 'oxygenMo')"
              dot
              inline
              color="error"
            />{{ item.oxygenMo }}
          ...
          <td>
            <v-badge
              v-if="checkValue(item.oxygenEv, 'oxygenEv')"
              dot
              inline
              color="error"
            />{{ item.oxygenEv }}
    ...
    <v-dialog v-model="dlgOpen">
      <RegisterInpatientStatus :patient="selectData" @dlg-close="fncDlgClose" />
    </v-dialog>
  </v-card>
</template>
...
export default {
  components: { RegisterInpatientStatus },
  props: {
    byoto: {
      type: String,
    },
  ...
  created: function () {
    let instance = new Promise(this.getData);
    instance = instance.then(this.addList);
    console.log("todayList:", this.todayList);
  ...
  methods: {
    async getData(resolve) {
      ...
      querySnapshot1.forEach((doc) => {
        this.list.push({
          ...
          age: this.getAge(doc.data().birthday),
    ...
    addList() {
      this.list.forEach((item) => {
        const result = this.todayList.find(
          (element) => element.patientId == item.id
        );
        if (result == undefined) {
          item.tempMo = "";
          item.tempEv = "";
